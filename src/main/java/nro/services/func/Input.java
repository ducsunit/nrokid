...
//                    }
                ...
                case QUY_DOI_COIN_1:
                    ...
                    if (goldTrade1 % 1000 == 0) {
                        if (goldTrade1 < 10000 || goldTrade1 >= 1000001) {
                            Service.getInstance().sendThongBao(player, "|7|Quá giới hạn mỗi lần tối thiểu 10.000 và tối đa 1.000.000");
                        } else if (player.getSession().vnd >= goldTrade1) {
                            PlayerDAO.subVnd(player, goldTrade1);
                            player.tongnap += goldTrade1;
                            Item thoiVang = ItemService.gI().createNewItem((short) 457, (kmnap1 * (goldTrade1 / 200)));
                            InventoryService.gI().addItemBag(player, thoiVang, 9999);
                            InventoryService.gI().sendItemBags(player);
                            Service.getInstance().sendThongBao(player, "Bạn nhận được " + Util.format((kmnap1 * (goldTrade1 / 200)))
                ...
                case QUY_DOI_XU_VANG:
                    ...
                    if (goldTrade2 % 1000 == 0) {
                        if (goldTrade2 < 10000 || goldTrade2 >= 1000001) {
                            Service.getInstance().sendThongBao(player, "|7|Quá giới hạn mỗi lần tối thiểu 10.000 và tối đa 1.000.000");
                            ...
                            Service.getInstance().sendThongBao(player, "|7|Số Coin của bạn là " + player.getSession().vnd + " không đủ để quy "
                                    + " đổi " + (goldTrade2 / 100) + " Xu vàng" + " " + "bạn cần thêm " + (player.getSession().vnd - goldTrade2));
                        }
                    } else {
                        Service.getInstance().sendThongBao(player, "|7|Số Coin nhập phải là bội số của 1000");
                    }
