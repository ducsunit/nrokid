        ...
        if (player.getSession() != null && player.isAdmin()) {
            ...
            if (text.equals("admin")) {
                ...
                NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_ADMIN, -1, str,
                        ...
                        "Cập Nhật\nThông Báo", "Buff Item Option", "Đổi hành tinh", "Tặng\nDanh hiệu");
                return;
...
//            Send_Info_NV(pl);
    ...
    public void sendTitleRv(Player player, Player p2, int id) {
        ...
        try {
            ...
            if (id == 900) {
                if (player.gender == 0 && player.nPoint.power >= 500_000_000_000L) {
                    me.writer().writeShort(94);
                } else if (player.gender == 1 && player.nPoint.power >= 500_000_000_000L) {
                    me.writer().writeShort(93);
                } else if (player.gender == 2 && player.nPoint.power >= 500_000_000_000L) {
...
//    public void newpassword(int status) {
    ...
    public void InfoPetGoc(Player pl) {
        if (pl != null && pl.pet != null) {
            ...
            try {
                ...
                msg.writer().writeLong(pl.pet.nPoint.mpg); //hpfull
                msg.writer().writeLong(pl.pet.nPoint.dameg); //mp
                msg.writer().writeLong(pl.pet.nPoint.defg); //mpfull
                ...
                for (int i = 0; i < pl.pet.playerSkill.skills.size(); i++) {
                    if (pl.pet.playerSkill.skills.get(i).skillId != -1) {
                        ...
                        if (i == 1) {
                            ...
                            msg.writer().writeShort(-1);
                            msg.writer().writeUTF("Cần đạt sức mạnh 20 tỷ\nđể mở");
                        }
                    }
                }
...
//            msg.writer().writeShort(0);
    ...
    public void showTopPower(Player player, int top) {
        ...
        try {
            msg.writer().writeByte(0);
            msg.writer().writeUTF(top == TOP_SUCMANH ? "Top Sức Mạnh" : top == TOP_DETU ? "Top Đệ tử" : top == TOP_NHIEMVU ? "Top Nhiệm vụ" : "Top Nạp");
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (top == TOP_DETU && pl.pet != null) {
                    msg.writer().writeUTF("Loại: " + (pl.pet.typePet == 0 ? "Đệ tử Thường" : pl.pet.typePet == 1 ? "Mabu" : pl.pet.typePet == 2 ? "Berus" : pl.pet.typePet == 3 ? "Zeno" : "Luffy Nika"));
                } else {
                    msg.writer().writeUTF(Client.gI().getPlayer(pl.id) != null ? "Online" : "Chưa Online");
                }
                msg.writer().writeUTF(top == TOP_SUCMANH ? "Sức mạnh: " + Util.numberToMoney(pl.nPoint.power) + "\n Chuyển sinh: " + pl.chuyensinh
    ...
    public void Mabu14hAttack(Boss mabu, Player plAttack, int x, int y, byte skillId) {
        ...
        try {
            ...
            if (skillId == 1) {
                msg.writer().writeByte(1);
                int dame = Util.DoubleGioihan(plAttack.injured(mabu, (int) (mabu.nPoint.getDameAttack(false) * (skillId == 1 ? 1.5 : 1)), false, false));
