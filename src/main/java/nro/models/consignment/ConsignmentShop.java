...
//                    consign(player, itemID, monneyType, money, quantity);
                    consign(player, itemID, monneyType, money, quantity, (short) 457, 1);
                }
                break;
                case BUY: {
                    short itemID = dis.readShort();
    ...
    private void consign(Player player, short itemID, byte monneyType, long money, int quantity) {
        if (quantity < 0 || quantity > 99) {
            Service.getInstance().sendThongBao(player, "Chỉ có thể kí gửi tối đa x99");
            return;
        }
        Item thoi_vang = InventoryService.gI().findItemBagByTemp(player, 457);

        if (money <= 0 || money >= 100_000_000_000_000L) {
            Service.getInstance().sendThongBao(player, "Số tiền không hợp lệ");
            return;
        }
        if (thoi_vang == null || thoi_vang.quantity <= 0) {
            Service.getInstance().sendThongBao(player, "Số tiền không hợp lệ");
            return;
        }
    ...
    private void consign(Player player, short itemID, byte moneyType, int money, int quantity, short itemExchangeID, int exchangeQuantity) {
        if (quantity < 0 || quantity > 99) {
            Service.getInstance().sendThongBao(player, "Chỉ có thể kí gửi tối đa x99");
            return;
        }
        if (money <= 0 && exchangeQuantity <= 0) {
            Service.getInstance().sendThongBao(player, "Thông tin giao dịch không hợp lệ");
            return;
        }
        if (findItemConsign(player.id, itemID) != null) {
            Service.getInstance().sendThongBao(player, "Không thể kí gửi nhiều vật phẩm giống nhau");
        ...
        if (itemExchangeID > 0 && exchangeQuantity > 0) {
            Item exchangeItem = InventoryService.gI().findItem(player, itemExchangeID, exchangeQuantity);
            if (exchangeItem == null || exchangeItem.quantity < exchangeQuantity) {
                Service.getInstance().sendThongBao(player, "Không đủ vật phẩm giao dịch");
                return;
            }
            InventoryService.gI().subQuantityItemsBag(player, exchangeItem, exchangeQuantity);
        }
        ...
        if (moneyType == 0) {
            ...
            consignmentItem.setItemExchangeID(itemExchangeID);
            consignmentItem.setExchangeQuantity(exchangeQuantity);
        }
    ...
    public void nextPage(Player player, byte tab, int page) {
        ...
        try {
            ...
            for (ConsignmentItem item : list) {
                ...
                ds.writeShort(item.template.id);
                ds.writeLong(item.getPriceGold());
                ds.writeLong(item.getPriceGem());
        ...
        try {
            ...
            for (byte i = 0; i < tabLength - 1; i++) {
                ...
                for (ConsignmentItem item : lists) {
                    ds.writeLong(item.getPriceGold());
                    ds.writeLong(item.getPriceGem());
                    ds.writeShort(item.getItemExchangeID()); // ID vật phẩm giao dịch
                    ds.writeInt(item.getExchangeQuantity()); // Số lượng vật phẩm giao dịch
                    ds.writeByte(0);
