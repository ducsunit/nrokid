            ...
            synchronized (itemMap) {
                if (!itemMap.isPickedUp) {
                    if (itemMap.playerId == player.id || itemMap.playerId == -1) {
                        if (itemMap.itemTemplate.id == 934 && player.bongtai >= 400) {
                            Service.getInstance().sendThongBao(player, "Mỗi ngày chỉ nhặt được 400 Mảnh hồn");
                            return;
                        } else if (itemMap.itemTemplate.id == 934 && player.bongtai < 400) {
                            player.bongtai++;
                            InventoryService.gI().sendItemBags(player);
                            Service.getInstance().sendToAntherMePickItem(player, itemMapId);
                        }
//                        if (this.map.mapId != 212) {
//                            if (isManhTS(itemMap) && player.thiensu >= 200) {
//                                Service.getInstance().sendThongBao(player, "Mỗi ngày chỉ nhặt được 200 Mảnh thiên sứ");
//                                return;
//                            } else if (isManhTS(itemMap) && player.thiensu < 200) {
//                                player.thiensu++;
//                                InventoryService.gI().sendItemBags(player);
//                                Service.getInstance().sendToAntherMePickItem(player, itemMapId);
//                            }
                                ...
                                switch (itemType) {
                                    ...
                                    default:
                                        switch (item.template.id) {
                                            case 457:
                                                ServerLog.logThoiVangDrop(player.name, itemMap.itemTemplate.name, itemMap.quantity);
                                                break;
                                            case 73:
                                                msg.writer().writeUTF("");
...
//                }
    ...
    private void infoPlayer(Player plReceive, Player plInfo) {
        ...
        if (!plInfo.isBoss && !plInfo.isPet && !plInfo.isMiniPet && !(plInfo instanceof TestDame)) {
